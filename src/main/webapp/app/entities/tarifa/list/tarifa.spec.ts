...
import { sampleWithRequiredData } from '../tarifa.test-samples';
...
describe('Tarifa Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 15454 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tarifas()[0]).toEqual(expect.objectContaining({ id: 15454 }));
  });

  describe('trackId', () => {
    it('should forward to tarifaService', () => {
      const entity = { id: 15454 };
      vitest.spyOn(service, 'getTarifaIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tarifasParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
